...
function cannonMovement() {
	...
	if (cannons.rotating.clockwise) {
		if (playingField.activeCannon.rotation.y > -Math.PI / 2) playingField.activeCannon.rotation.y -= rotationSpeed;
	}
...
function cannonballsMovement() {
	...
	for (var i = 0; i < CannonBalls.length; i++) {
		if (CannonBalls[i].movement != null) var perpendicular = new THREE.Vector3(CannonBalls[i].movement.x, CannonBalls[i].movement.y, CannonBalls[i].movement.z);
		...
		CannonBalls[i].position.addScaledVector(CannonBalls[i].movement, -CannonBalls[i].speed * delta);
	}
}
